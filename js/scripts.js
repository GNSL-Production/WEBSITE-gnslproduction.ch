...
// ── Hero tagline: per-character wave animation ─────────────────
const headline = document.querySelector('.hero-headline');
if (headline) {
  const words = headline.textContent.trim().split(' ');
  headline.innerHTML = words.map((word, wi) =>
    word.split('').map((char, ci) => {
      const delay = (wi * 0.14 + ci * 0.055).toFixed(2);
      return `<span style="animation-delay:${delay}s">${char}</span>`;
    }).join('') + (wi < words.length - 1 ? ' ' : '')
  ).join('');
}
...
// ── iPhone: reveal iframe on click ───────────────────────────
document.querySelectorAll('.btnView').forEach((btn, i) => {
  const iframe    = document.querySelectorAll('.iframeMobile')[i];
  const container = document.querySelectorAll('.iphone')[i];
  let opened = false;
...
document.querySelectorAll('.bento, .pricing-grid, .delivery-grid').forEach(grid => {
